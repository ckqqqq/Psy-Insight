...
if eval_arg not in eval_list:
    raise ValueError()

lora_path=None
baseline_lora_path=""
dialog_lora_path="/home/ckqsudo/code2024/Mistral-7b-finetune/fuck7_6_5_unpacking_pure_dialog_3/checkpoint-570"
reasoning_lora_path="/home/ckqsudo/code2024/Mistral-7b-finetune/fuck7_6_5_unpacking__reasoning_3/checkpoint-228"
reasoning_observation_lora_path="/home/ckqsudo/code2024/Mistral-7b-finetune/fuck7_6_5_unpacking__observation_reasoning_3/checkpoint-228"
mix_lora_path="/home/ckqsudo/code2024/Mistral-7b-finetune/trl_standard_output/fuck8_6_8_unpacking__continue_training/checkpoint-120"
...
base_test=d_test
mix_test="/home/ckqsudo/code2024/Mistral-7b-finetune/train_and_test_data/test_dataset_pure_dialog.jsonl"
...
if eval_arg in eval_list:
    ...
    print(eval_arg,lora_path,test_dataset_path.split("/")[-1])
    ...
    for idx,msg_item in enumerate(test_dataset['train']):
        ...
        while True:
            while len(msg_list)!=0 and 'user' in msg_list[-1]['role']:
                msg_list=msg_list[:-1]
            if len(msg_item)!=0 and 'assistant' in msg_list[-1]['role']:
...
for idx,msg_item in enumerate(format_datset):
    ...
    with torch.inference_mode():
        with torch.no_grad():
            ...
            print(str_output)
            beg_idx=str_output.find("Therapist:")
            if beg_idx==-1:
                beg_idx=str_output.find("Therapist")
...
for item in result:
    ...
    # print(item['hyps'].startswith("Therapist:"),item['ref'].startswith("Therapist:"))
...
# %%
print_prompt=tokenizer.apply_chat_template(item[0]["messages"], tokenize=False)
print_prompt
